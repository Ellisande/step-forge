...
The step phase (`stepType`) is normally the `givenBuilder`/`whenBuilder`/`thenBuilder` identifier in the call chain, but the extractor also resolves the **pre-bound** styles (`resolveReExportedCall` in `stepExtractor.ts`): a re-exported bound `.statement` (`export const Given = givenBuilder<T>().statement`, used as `Given("x").step(...)` — the README "Simpler Step Definitions" pattern) and a destructured `createBuilders()` object (`const { Given } = createBuilders<...>()`, used as `Given.statement(...)`/`Given.variables(...)`). These trace the origin identifier's declaration through **import aliases** (`getAliasedSymbol`), so builders defined in one module and imported into the step file resolve correctly — this needs the type checker, so it triggers the checked retry. The unusual namespace form (`const B = createBuilders(); B.Given.statement(...)`) is not resolved. Fixtures: `prebound-builders.ts` + `prebound-steps.ts`.
